...
add_action('admin_menu', 'accessibly_otm_init_menu');
 ...
 * @return void
 */
function accessibly_otm_init_menu()
{
    ...
    add_menu_page(__('Accessibly App', 'otm-accessibly'), __('Accessibly App', 'otm-accessibly'), 'manage_options', 'accessibly-otm', 'accessibly_otm_admin_page', 'data:image/svg+xml;base64,' . base64_encode($accessiblyIcon), 100);
    add_submenu_page('accessibly-otm',  __('My Plan', 'otm-accessibly'), __('My Plan', 'otm-accessibly'), 'manage_options',  __('accessibly-plan', 'otm-accessibly'), 'accessibly_otm_my_plan_page', 100);
    add_submenu_page('accessibly-otm',  __('Accessibly Statement', 'otm-accessibly'), __('Accessibly Statement', 'otm-accessibly'), 'manage_options',  __('accessibly-statement', 'otm-accessibly'), 'accessibly_otm_statement_page', 100);
}

add_filter('plugin_action_links_'. plugin_basename(__FILE__), 'accessibly_otm_settings_link');
function accessibly_otm_settings_link($links)
{
    ...
    $urlMyPlan = esc_url(add_query_arg(
        'page',
        'accessibly-plan',
        get_admin_url() . 'admin.php'
    ));
    // Create the link.
    $settings_link = "<a href='$url'>" . __('Settings', 'otm-accessibly') . '</a>';
    $upgrade_link = "<a href='$urlMyPlan' style='color: #0eb015'>" . __('Upgrade To Premium', 'otm-accessibly') . '</a>';
...
function accessibly_otm_activation_redirect($plugin)
{
    if ($plugin == plugin_basename(__FILE__)) {
        exit(wp_redirect(admin_url('admin.php?page=accessibly-otm')));
    }
}
add_action('activated_plugin', 'accessibly_otm_activation_redirect');
 ...
 * @return void
 */
function accessibly_otm_admin_page()
{
 ...
 * @return void
 */
function accessibly_otm_my_plan_page()
{
    require_once plugin_dir_path(__FILE__) . 'templates/app.php';
}
 ...
 * @return void
 */
function accessibly_otm_statement_page()
{
    require_once plugin_dir_path(__FILE__) . 'templates/app.php';
}

add_action('admin_enqueue_scripts', 'accessibly_otm_admin_enqueue_scripts');
 ...
 * @return void
 */
function accessibly_otm_admin_enqueue_scripts()
{
    $screen = get_current_screen();
    if (strpos($screen->base, "accessibly") !== false) {
...
function accessibly_otm_save_token()
{
    $token = $_POST['token'];
...
add_action('wp_ajax_nopriv_accessibly_otm_save_token', 'accessibly_otm_save_token');
add_action('wp_ajax_accessibly_otm_save_token', 'accessibly_otm_save_token');
 ...
 * @since    1.0.0
 */
function accessibly_otm_ac_plugin()
{
...
add_action('wp_enqueue_scripts', 'accessibly_otm_enqueue_scripts');
/**
     * Register the stylesheets for the public-facing side of the site.
     *
     * @since    1.0.0
     */
function accessibly_otm_enqueue_scripts()
{
    $url = "https://dash.accessibly.app/api/get-domain-id";
    ...
    wp_enqueue_script(
        "accessibly-widget",
        "https://dash.accessibly.app/widget/" . $data['id'] . "/autoload.js"
    );
}

register_uninstall_hook(__FILE__, 'otm_accessibly_uninstall');

function otm_accessibly_uninstall()
{
    $token = get_option('otm_accessibly_token');
    ...
    $response = $client->post("https://dash.accessibly.app/api/delete-wordpress", [
        'headers' => [
            'Authorization' => 'Bearer ' . $token,
        ],
    ]);
